...
GLuint bunny_vao_id;
GLuint program_id;
GLuint program_id_no_tesselation;
...
void display() {
  ...
  else
  {
	  ...
	  glDrawArrays(GL_PATCHES, 0, 4);
	  //glDrawArrays(GL_LINE, 0, 4);TEST_OPENGL_ERROR();
...
std::string load(const std::string &filename) {
  ...
  while(std::getline(input_src_file, ligne)) {
...
bool init_shaders() {
  ...
  std::string control_src = load("..//tessellationcontrol.shd");
  std::string shaders_src = load("../tessellationshaders.shd");
  std::string vertex_tessellation_src = load("../vertexnotesselation.shd");
  GLuint shader_id[5];
  ...
  char *control_shd_src = (char*)std::malloc(control_src.length()*sizeof(char));
  char *shaders_shd_src = (char*)std::malloc(shaders_src.length()*sizeof(char));
  char *vertex_tessellation_shd_src = (char*)std::malloc(vertex_tessellation_src.length() * sizeof(char));
  vertex_src.copy(vertex_shd_src,vertex_src.length());
  fragment_src.copy(fragment_shd_src,fragment_src.length());
  control_src.copy(control_shd_src,control_src.length());
  shaders_src.copy(shaders_shd_src,shaders_src.length());
  vertex_tessellation_src.copy(vertex_tessellation_shd_src, vertex_tessellation_src.length());

  shader_id[0] = glCreateShader(GL_VERTEX_SHADER);TEST_OPENGL_ERROR();
  shader_id[1] = glCreateShader(GL_VERTEX_SHADER);TEST_OPENGL_ERROR();
  ...
  glShaderSource(shader_id[1], 1, (const GLchar**)&(vertex_tessellation_shd_src), 0);TEST_OPENGL_ERROR();
  glShaderSource(shader_id[2], 1, (const GLchar**)&(control_shd_src), 0);TEST_OPENGL_ERROR();
  glShaderSource(shader_id[3], 1, (const GLchar**)&(shaders_shd_src), 0);TEST_OPENGL_ERROR();
  ...
  for(int i = 0 ; i < 5 ; i++) {
	...
	std::cout << "SHADER " << i << ": " << shader_log << std::endl;
	std::free(shader_log);
      }
      std::free(vertex_shd_src);
      std::free(fragment_shd_src);
      std::free(control_shd_src);
      std::free(shaders_shd_src);
	  std::free(vertex_tessellation_shd_src);
  ...
  program_id_no_tesselation = glCreateProgram();TEST_OPENGL_ERROR();
  if (program_id==0) return false;
  if (program_id_no_tesselation == 0) return false;
  for(int i = 0 ; i < 5 ; i++) {
	 if (i !=1)
		glAttachShader(program_id, shader_id[i]);TEST_OPENGL_ERROR();
  }
  glLinkProgram(program_id);TEST_OPENGL_ERROR();
  glLinkProgram(program_id_no_tesselation);TEST_OPENGL_ERROR();
